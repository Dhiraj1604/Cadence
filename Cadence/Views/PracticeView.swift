    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            GeometryReader { geo in
                ...
                .frame(height: 24 * Device.scale, alignment: .center)
            }
            .frame(height: 24 * Device.scale)
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 3) {
            ForEach(Array(samples.enumerated()), id: \.0) { _, sample in
                let barWidth: CGFloat = 5 * Device.scale
                let flatHeight: CGFloat = 4 * Device.scale
                let activeHeight: CGFloat = max(flatHeight, sample * 48 * Device.scale + flatHeight)
                let barHeight: CGFloat = isSpeaking ? activeHeight : flatHeight
                let barAnimation: Animation = isSpeaking
                ...
                RoundedRectangle(cornerRadius: 2)
                    ...
                    .animation(barAnimation, value: sample)
            }
        }
        .frame(height: 56 * Device.scale)
        .frame(maxWidth: .infinity)
    }
}
...
struct PracticeView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $viewModel.showEndSheet) {
            EndSessionSheet(
                duration: session.duration,
                onKeepGoing: { viewModel.showEndSheet = false },
                onEndSession: {
                    viewModel.showEndSheet = false
                    coachEngine.stop()
                    cameraManager.stop()
                    session.endSession(
                        wpm: coachEngine.wpm,
        ...
        .onChange(of: coachEngine.fillerWordCount) { _, v in viewModel.checkForLiveTip(fillers: v, duration: session.duration) }
        .onChange(of: coachEngine.cognitiveLoadWarning) { _, v in viewModel.checkCognitiveLoadWarning(warning: v, duration: session.duration) }
        .onChange(of: coachEngine.rollingWPM) { _, s in
            if s > 175 { viewModel.checkWPMTip(wpm: s, duration: session.duration) }
            if s > 0 && s < 100 { viewModel.checkSlowTip(wpm: s, duration: session.duration) }
        }
        .onChange(of: cameraManager.isMakingEyeContact) { _, v in if !v { viewModel.checkEyeContactTip(duration: session.duration) } }
    }
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 10) {
                MetricTile(
                    ...
                    badge: viewModel.wpmStatus(wpm: coachEngine.rollingWPM),
                    color: viewModel.wpmColor(wpm: coachEngine.rollingWPM)
                )
                MetricTile(
                    icon: "exclamationmark.bubble.fill",
                    label: "Fillers",
                    value: "\(coachEngine.fillerWordCount)",
                    badge: viewModel.fillerStatus(count: coachEngine.fillerWordCount),
                    color: viewModel.fillerColor(count: coachEngine.fillerWordCount)
                )
                MetricTile(
                    icon: "waveform.path",
                    label: "Rhythm",
                    ...
                    badge: viewModel.rhythmStatus(stability: coachEngine.rhythmStability),
                    color: viewModel.rhythmColor(stability: coachEngine.rhythmStability)
                )
            }
            .padding(.horizontal, 20)
            .padding(.bottom, 10)
    ...
    private var bottomControls: some View {
        VStack(spacing: 0) {
            if viewModel.tipVisible, let tip = viewModel.liveTip {
        ...
        .animation(.spring(response: 0.4, dampingFraction: 0.8), value: viewModel.tipVisible)
    }
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 8) {
                    Text("Practice Session")
                        .font(.system(size: Device.isPad ? 24 : 17, weight: .semibold))
                        .foregroundStyle(.white)
                ...
                Text(timeString(from: session.duration))
                    .font(.system(size: Device.isPad ? 40 : 28, weight: .bold, design: .monospaced))
                    .foregroundStyle(LinearGradient.cadencePrimary)
            ...
            if coachEngine.wpm > 0 && coachEngine.spontaneityScore > 0 {
                VStack(spacing: 1) {
                    Text("STYLE")
                        .font(.caption2.weight(.heavy))
                        .foregroundStyle(Color.white.opacity(0.25))
                    Text(viewModel.spontaneityLabel(score: coachEngine.spontaneityScore))
                        .font(.caption.weight(.bold))
                        .foregroundStyle(viewModel.spontaneityColor(score: coachEngine.spontaneityScore))
                }
                .padding(.horizontal, 10).padding(.vertical, 6)
                .background(viewModel.spontaneityColor(score: coachEngine.spontaneityScore).opacity(0.1))
        ...
        return VStack(alignment: .leading, spacing: 6) {
            ...
            if isEmpty {
                ...
                ScrollViewReader { proxy in
                    ...
                    .frame(maxHeight: Device.isPad ? 140 : 90)

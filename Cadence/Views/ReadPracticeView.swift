        ...
        VStack(spacing: 0) {
            ...
            if let prompt = engine.selectedPrompt {
                ...
                .overlay(RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .strokeBorder(prompt.color.opacity(0.25), lineWidth: 1))
                .padding(.horizontal, 20)
                .opacity(appeared ? 1 : 0)
...
struct ReadResultsView: View {
    ...
    private func coachTipCard(_ tip: (icon: String, color: Color, title: String, message: String)) -> some View {
        ...
        .background(.ultraThinMaterial)
        .clipShape(RoundedRectangle(cornerRadius: 14, style: .continuous))
        .overlay(RoundedRectangle(cornerRadius: 14, style: .continuous).strokeBorder(tip.color.opacity(0.28), lineWidth: 1))
    }
    ...
    private func fillerCard(_ result: ReadingAnalysisResult) -> some View {
        ...
        .background(.ultraThinMaterial)
        .clipShape(RoundedRectangle(cornerRadius: 14, style: .continuous))
        .overlay(RoundedRectangle(cornerRadius: 14, style: .continuous).strokeBorder(Color.cadenceWarn.opacity(0.25), lineWidth: 1))
    }
}
